...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        f1, cache_f1 = affine_relu_forward(X, W1, b1)
        scores, cache_f2 = affine_forward(f1, W2, b2)
        ...
        df1, dW2, grads['b2'] = affine_backward(dS, (f1, W2, b2))
        _, dW1, grads['b1'] = affine_relu_backward(df1, cache_f1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        df, dW, grads['b'+num] = affine_backward(dS, cache_layers['affine'+num])
        grads['W'+num] = dW+self.reg*self.params['W'+num]
        l -= 1
        # Middle laysers and the 1st layer
        while l > 0:
            num = str(l)
            loss += 0.5*self.reg*np.sum(self.params['W'+num]**2)
            dp = df
